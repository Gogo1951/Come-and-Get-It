...
--[[
    Per-argument byte cap. 255 holds a full line (for example a UI error string
    with an embedded item link) while still bounding a runaway argument. Smaller
    caps such as 64 truncate a link mid-name and collapse the entry to a sliver.
]]
...
--[[
    For every event Come & Get It registers (ns.EVENT_NAMES, exported by
...
--[[
    Live read of the state Core's UI_ERROR_MESSAGE handler depends on: the
    detection config it matches against, the suppression gates that most often
    explain "nothing happened," and the exact C_Map chain AnnounceNode walks. An
    existence check can't prove the map calls return a usable mapID and position,
